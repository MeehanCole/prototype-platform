...
const FEATURE_LABELS: Record<string, { type: "A" | "B" | "M" | "R"; title: string; desc: string }> = {
  "B.01": { type: "B", title: "用户管理 - 列表与筛选", desc: "子账号列表，支持账号名/邮箱/手机号/UASS/员工号筛选查询" },
  "B.02": { type: "B", title: "用户管理 - 子账号CRUD", desc: "添加/查看/编辑/删除子账号，含密码方式配置" },
  "B.03": { type: "B", title: "用户管理 - 批量操作", desc: "批量删除、导入用户数据、导出列表、添加到用户组" },
  "B.04": { type: "B", title: "用户组管理 - 列表与CRUD", desc: "用户组列表，含名称/备注/用户数/创建时间，支持新建/编辑/删除" },
  "B.05": { type: "B", title: "用户组管理 - 编辑页", desc: "基本信息表单 + 双Tab（已添加用户/关联策略），支持移出用户和移除策略" },
  "B.06": { type: "B", title: "策略管理 - 列表与CRUD", desc: "策略列表，区分预置/自定义策略，支持新建和删除" },
  "B.07": { type: "B", title: "策略管理 - 详情页", desc: "策略信息卡 + 双Tab（配置/关联用户组），配置Tab含产品树+接口列表" },
  "B.08": { type: "B", title: "云API密钥管理", desc: "签发新密钥、明文/掩码切换、禁用/启用、删除密钥" },
  "B.09": { type: "B", title: "UASS认证管理", desc: "两步开启流程：配置参数 → 开启开关，支持确认弹窗" },
  "B.10": { type: "B", title: "账号纳管", desc: "纳管账号列表、新增/编辑/删除，含密钥管理" },
  "B.11": { type: "B", title: "个人中心", desc: "账号信息、账号权限、安全设置三张卡片，支持编辑部分信息" },
};
...
const FEATURE_DOCS: Record<string, string> = {
  ...
  "B.08": extractSection(prdRaw, "FR-8"),
  "B.09": extractSection(prdRaw, "FR-9"),
  "B.10": extractSection(prdRaw, "FR-10"),
  "B.11": extractSection(prdRaw, "FR-11"),
};
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex items-center gap-0.5 h-4 px-1 rounded-full bg-background/95 ring-1 text-[9px] transition-colors z-10 whitespace-nowrap font-medium shadow-sm ${hasDoc ? "ring-primary/40 text-primary hover:bg-primary/10 hover:ring-primary/60 cursor-pointer" : "ring-border text-muted-foreground hover:bg-accent cursor-help"}`}
      >
...
function BtnDefault({ children, onClick, danger, style }: { children: ReactNode; onClick?: () => void; danger?: boolean; style?: CSSProperties }) {
  return (
    <button
      onClick={onClick}
...
function FieldInput({ value, onChange, placeholder, disabled, style }: { value: string; onChange?: (v: string) => void; placeholder?: string; disabled?: boolean; style?: CSSProperties }) {
  return (
    <input
      value={value}
...
function ConfirmModal({ title, content, onClose, onConfirm }: { title: string; content: ReactNode; onClose: () => void; onConfirm: () => void }) {
  return (
...
function UserListPage({
  ...
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #f0f0f0", padding: "13px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left", whiteSpace: "nowrap" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", verticalAlign: "middle" };
...
function UserEditPage({ user, onBack, onAddToGroup }: { user: User; onBack: () => void; onAddToGroup: () => void }) {
  ...
  const lbl: CSSProperties = { fontSize: 14, color: "rgba(0,0,0,0.65)", fontWeight: 500, whiteSpace: "nowrap", paddingRight: 12, minWidth: 90 };
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #f0f0f0", padding: "8px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "12px 16px", fontSize: 14, color: "rgba(0,0,0,0.85)" };

  const Row2 = ({ label, children }: { label: string; children: ReactNode }) => (
    <div className="flex items-center">
...
function EditAccountModal({ onClose }: { onClose: () => void }) {
  ...
  const reqRow = (label: string, node: ReactNode) => (
...
function AddSubAccountModal({ onClose }: { onClose: () => void }) {
  ...
  const lbl: CSSProperties = { fontSize: 14, color: "rgba(0,0,0,0.85)", fontWeight: 500, whiteSpace: "nowrap", paddingRight: 12, minWidth: 96, flexShrink: 0 };
  const req = (label: string, node: ReactNode) => (
  ...
  const opt = (label: string, node: ReactNode) => (
    <div className="flex items-center" style={{ marginBottom: 20 }}>
      <span style={lbl}>{label}</span>
      <div className="flex-1 min-w-0">{node}</div>
    </div>
...
function AddToGroupModal({ onClose }: { onClose: () => void }) {
  ...
  const thS: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #f0f0f0", padding: "9px 12px", fontSize: 12, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left" };
  const tdS: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "6px 12px", fontSize: 12, color: "rgba(0,0,0,0.85)", height: 35 };
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.45)" }}
      onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}
    >
...
function UserGroupListPage({ groups, setGroups, onNewGroup, onEditGroup, onAddUserDirect, onAssocPolicy }: {
  ...
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "16px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left", whiteSpace: "nowrap" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 55, verticalAlign: "middle" };
  const lbl: CSSProperties = { fontSize: 14, color: "rgba(0,0,0,0.65)", fontWeight: 500, whiteSpace: "nowrap", paddingRight: 12 };
...
function UserGroupEditPage({ group, onBack, onAddUser }: { group: UserGroup; onBack: () => void; onAddUser: () => void }) {
  ...
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "16px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 54, verticalAlign: "middle" };
  const lbl: CSSProperties = { fontSize: 14, color: "rgba(0,0,0,0.65)", fontWeight: 500, whiteSpace: "nowrap", paddingRight: 12, minWidth: 80 };
...
function NewGroupModal({ onClose, onConfirm }: { onClose: () => void; onConfirm: (name: string, remark: string) => void }) {
  ...
  const lbl: CSSProperties = { fontSize: 14, color: "rgba(0,0,0,0.65)", fontWeight: 500, whiteSpace: "nowrap", paddingRight: 12, minWidth: 72 };
  return (
...
function AddUserModal({ onClose }: { onClose: () => void }) {
  ...
  const thL: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "12px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left" };
  const tdL: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 47, verticalAlign: "middle" };
...
function CloudApiPage({ apiKeys, setApiKeys, onNewKey, onDeleteKey }: {
  ...
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "16px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left", whiteSpace: "nowrap" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 57, verticalAlign: "middle" };
  return (
    <div className="flex-1 overflow-auto bg-[#f0f2f5] p-6 flex flex-col gap-4">
      <div className="flex items-center gap-2">
...
function PolicyListPage({ policies, setPolicies, onNewPolicy, onViewPolicy, onDeletePolicy }: {
  ...
  const lbl: CSSProperties = { fontSize: 14, color: "rgba(0,0,0,0.65)", fontWeight: 500, whiteSpace: "nowrap", paddingRight: 12 };
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "16px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left", whiteSpace: "nowrap" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 58, verticalAlign: "middle" };
...
function PolicyDetailPage({ policy, onBack, onAssociateGroup }: { policy: Policy; onBack: () => void; onAssociateGroup: () => void }) {
  ...
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "12px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 54, verticalAlign: "middle" };
...
function CustodyEditPage({ account, isNew, onBack }: { account: CustodyAccount | null; isNew: boolean; onBack: () => void }) {
  ...
  const editRow = (label: string, required: boolean, node: ReactNode) => (
    <div className="flex items-center" style={{ marginBottom: 20 }}>
...
function AccountCustodyPage({ onEdit, onNew }: { onEdit: (a: CustodyAccount) => void; onNew: () => void }) {
  ...
  const thStyle: CSSProperties = { background: "#fafafa", borderBottom: "1px solid #e8e8e8", padding: "13px 16px", fontSize: 14, fontWeight: 500, color: "rgba(0,0,0,0.85)", textAlign: "left", whiteSpace: "nowrap" };
  const tdStyle: CSSProperties = { borderBottom: "1px solid #f0f0f0", padding: "0 16px", fontSize: 14, color: "rgba(0,0,0,0.85)", height: 58, verticalAlign: "middle" };
...
export default function App({ __showFeat = false }: { __showFeat?: boolean } = {}) {
  ...
  return (
    <DocContext.Provider value={{ openDoc: setDocCode, showFeat: __showFeat }}>
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", minWidth: 1200, overflow: "hidden" }}>
